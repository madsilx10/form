...
async def x_retweet(auth_token: str, ct0: str, my_id: str):
    ...
    # Kalau 404: queryId kemungkinan expired, tangkap dulu sebelum safe_json
    ...
    print(f"    ↳ [DEBUG RT] status:{r.status_code} body:{r.text[:300]}")
    if r.status_code == 404:
        raise Exception(
            "RT 404 — queryId CreateRetweet expired. "
            "Buka DevTools (F12) di x.com, RT manual sambil rekam Network, "
            "cari request 'CreateRetweet', copy queryId dari URL-nya ke RETWEET_QUERY_ID."
        )
    if r.status_code == 401:
