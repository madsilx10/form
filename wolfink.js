...
const SUBMIT_URL = 'https://www.wolfink.fun/api/submit';
const REFERER_URL = 'https://www.wolfink.fun/';
const TARGET_SCREEN_NAME = 'WolfinkHQ';
...
async function processOne(acc) {
  ...
  try {
    ...
    if (followOk) {
      ...
      // lanjut aja ke submit, siapa tau udah follow duluan / error karena "already following"
    }
  } catch (err) {
  ...
  // 2. submit
  const s = await submitOne(acc);
  if (s.status === 200 && s.data && s.data.ok) {
    console.log(`✅ [${acc.index}] ${acc.handle} -> submitted (count=${s.data.count})`);
  } else {
    console.log(`❌ [${acc.index}] ${acc.handle} -> submit gagal:`, s.status, s.data);
  }
}
