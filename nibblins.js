...
const APPLY_URL = 'https://nibblins.xyz/api/apply';
const TARGET_SCREEN_NAME = 'NibblinsHQ';
const SPECIES_LIST = ['Goat', 'Cat', 'Deer', 'Monkey', 'Bunny'];
...
function loadAll() {
  ...
  const usernames = loadLines('usn.txt');
  const wallets = loadLines('wallet.txt');

  const n = Math.min(accounts.length, usernames.length, wallets.length);
  if (accounts.length !== usernames.length || accounts.length !== wallets.length) {
    console.log(`⚠️  Jumlah baris beda! akun=${accounts.length} usn=${usernames.length} wallet=${wallets.length}`);
  ...
  for (let i = 0; i < n; i++) {
    combined.push({
      ...
      ct0: accounts[i].ct0,
      handle: usernames[i].startsWith('@') ? usernames[i] : '@' + usernames[i],
...
function randomStatusId() {
  // angka 20 digit random di belakang (posisi depan fix: 20921611656...)
  // sesuai contoh: 2092161165694742533
  const prefix = '20';
  const suffix = String(Math.floor(Math.random() * 1e17)).padStart(17, '0');
  return prefix + suffix;
}

function buildLink(username) {
  const statusId = randomStatusId();
  return `https://x.com/${username}/status/${statusId}?s=20`;
}
...
async function getTargetUserId(acc) {
  ...
  if (res.status === 401) throw new Error('Token invalid/expired (401)');
...
async function followTarget(acc) {
  ...
  if (res.status === 403) return { status: 200, data: { already: true } };
...
async function applyOne(acc) {
  ...
  const link = buildLink(acc.username);
  const body = {
    species,
    handle: acc.handle,
    wallet: acc.wallet,
  ...
  let data;
  try { data = await res.json(); } catch { data = await res.text(); }
  return { status: res.status, data, species, link };
}
...
async function processOne(acc) {
  ...
  try {
    ...
    followOk = f.status === 200 && !(f.data?.errors);
    if (followOk) {
      console.log(`👤 [${acc.index}] ${acc.handle} -> follow OK`);
    } else {
      console.log(`⚠️  [${acc.index}] ${acc.handle} -> follow gagal:`, f.status, JSON.stringify(f.data).slice(0, 150));
    }
  } catch (e) {
    console.log(`⚠️  [${acc.index}] ${acc.handle} -> follow error: ${e.message}`);
  }

  // 2. Apply
  try {
    const a = await applyOne(acc);
    if (a.status === 200 && a.data?.ok) {
      console.log(`✅ [${acc.index}] ${acc.handle} -> apply OK | species: ${a.species}`);
      console.log(`   link: ${a.link}`);
    } else {
      console.log(`❌ [${acc.index}] ${acc.handle} -> apply gagal:`, a.status, a.data);
    }
  } catch (e) {
    console.log(`❌ [${acc.index}] ${acc.handle} -> apply error: ${e.message}`);
  }
}
...
async function main() {
  const all = loadAll();
  if (all.length === 0) {
    console.log('Data kosong, cek akun.txt / usn.txt / wallet.txt');
    return;
  }
  ...
  console.log('  3. Range (contoh: 3-end atau 3-10)\n');
  ...
  if (mode === '1') {
    ...
    const input = await ask('Format "x-y" atau "x-end": ');
