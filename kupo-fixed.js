...
async function twitterFollow(account) {
  ...
  // user_id wajib di query param (bukan body) — Twitter bedain behavior-nya
  const res = await fetch(`https://x.com/i/api/1.1/friendships/create.json?user_id=${KUPONFT_USER_ID}`, {
    method: "POST",
  ...
  if (res.status === 403) {
    // 403 = udah follow sebelumnya, Twitter emang balikin ini — bukan error
    console.log(`    ↳ Sudah follow (403), skip.`);
    return;
  }
