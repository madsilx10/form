...
# ── CONFIG ────────────────────────────────────────────────────────────────────
TWEET_ID         = "2087894490719940769"
KUPONFT_USER_ID  = "2087488162171871232"
...
# ── TWITTER ACTIONS ───────────────────────────────────────────────────────────
async def get_my_id(auth_token: str, ct0: str) -> str:
    h = x_headers(auth_token, ct0)
    async with AsyncSession(impersonate="chrome116") as s:
        r = await s.get("https://x.com/i/api/1.1/account/verify_credentials.json", headers=h)
...
async def x_follow(auth_token: str, ct0: str):
    ...
    async with AsyncSession(impersonate="chrome116") as s:
        r = await s.post(
            f"https://x.com/i/api/1.1/friendships/create.json?user_id={KUPONFT_USER_ID}",
            headers=h,
        )
...
async def x_retweet(auth_token: str, ct0: str, my_id: str):
    ...
    async with AsyncSession(impersonate="chrome116") as s:
        r = await s.post(
            f"https://x.com/i/api/2/users/{my_id}/retweets",
            headers=h,
            data=json.dumps({"tweet_id": TWEET_ID}),
        )
    data = r.json()
    errs = data.get("errors", [])
    if errs and errs[0].get("code") == 327:
        print("    ↳ Sudah RT, skip.")
        return
    if data.get("data", {}).get("retweeted") is True:
        return
    raise Exception(f"RT gagal: {json.dumps(data)[:200]}")

async def x_like(auth_token: str, ct0: str, my_id: str):
    h = {**x_headers(auth_token, ct0), "content-type": "application/json"}
    async with AsyncSession(impersonate="chrome116") as s:
        r = await s.post(
            f"https://x.com/i/api/2/users/{my_id}/likes",
            headers=h,
            data=json.dumps({"tweet_id": TWEET_ID}),
        )
    data = r.json()
    errs = data.get("errors", [])
    if errs and errs[0].get("code") == 139:
        print("    ↳ Sudah like, skip.")
        return
    if data.get("data", {}).get("liked") is True:
        return
...
async def process_account(acc: dict, idx: int, total: int, state: dict):
    ...
    try:
        ...
        if not is_done(state, key, "verify"):
            ...
            my_id = await get_my_id(acc["auth_token"], acc["ct0"])
