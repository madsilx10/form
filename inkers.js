...
const APPLY_URL = 'https://www.inkersnft.xyz/api/apply';
const TARGET_SCREEN_NAME = 'InkersonInk';
...
async function applyOne(acc) {
  const body = {
    username: acc.handle,
    wallet: acc.wallet,
    tasks: { follow: true, like: true, repost: true, tag: true },
  };
...
async function processOne(acc) {
  ...
  try {
    ...
    if (followOk) {
      ...
      // lanjut aja ke apply, siapa tau udah follow duluan / error karena "already following"
    }
  } catch (err) {
    console.log(`⚠️  [${acc.index}] ${acc.handle} -> follow error: ${err.message}`);
  }

  // 2. apply
  const a = await applyOne(acc);
  if (a.status === 200 && a.data && a.data.ok) {
    console.log(`✅ [${acc.index}] ${acc.handle} -> applied`);
  } else {
