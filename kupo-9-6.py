...
def safe_json(r) -> dict:
    """Parse JSON response, kasih error yang jelas kalau body kosong/bukan JSON."""
    text = r.text.strip() if hasattr(r, "text") else ""
    if not text:
        raise Exception(f"Response kosong (status {r.status_code}) — kemungkinan rate limit atau sesi expired")
    try:
        return json.loads(text)
    except Exception:
        raise Exception(f"Response bukan JSON (status {r.status_code}): {text[:200]}")

# ── CONFIG ────────────────────────────────────────────────────────────────────
TWEET_ID         = "2088343082278490504"
...
# ── HEADERS ───────────────────────────────────────────────────────────────────
def x_headers(auth_token: str, ct0: str, my_id: str = "") -> dict:
    cookie = f"auth_token={auth_token}; ct0={ct0}"
    if my_id:
        cookie += f"; twid=u%3D{my_id}"
    return {
        "cookie":                    cookie,
        "x-csrf-token":              ct0,
...
async def x_retweet(auth_token: str, ct0: str, my_id: str):
    ...
    h       = {**x_headers(auth_token, ct0, my_id), "content-type": "application/json"}
    payload = {
        "variables": {"tweet_id": TWEET_ID, "dark_request": False},
        "queryId":   RETWEET_QUERY_ID,
    }

    for attempt in range(1, 4):  # max 3 kali (226 kadang hilang sendiri)
        async with AsyncSession(impersonate="chrome124") as s:
            r = await s.post(
                f"https://x.com/i/api/graphql/{RETWEET_QUERY_ID}/CreateRetweet",
                headers=h,
                data=json.dumps(payload),
            )
        print(f"    ↳ [DEBUG RT] attempt:{attempt}/3 status:{r.status_code} body:{r.text[:300]}")

        if r.status_code == 404:
            raise Exception(
                "RT 404 — queryId expired. DevTools → Network → RT manual → "
                "cari request 'CreateRetweet' → copy hash di URL → ganti RETWEET_QUERY_ID."
            )
        if r.status_code == 401:
            raise Exception("RT 401 — token mati, ganti cookie.")
        if r.status_code == 403:
            text = r.text.strip()
            if not text:
                raise Exception("RT 403 — akun suspended atau tidak bisa RT.")
            data = json.loads(text)
            for err in data.get("errors", []):
                if err.get("code") == 327:
                    print("    ↳ Sudah RT, skip.")
                    return
            raise Exception(f"RT 403: {json.dumps(data)[:200]}")
        ...
        if errors:
            ...
            if err.get("code") == 327:
                ...
                return
            if err.get("code") == 344:
                print("    ↳ 344 — akun kena daily limit retweet. Skip, coba lagi besok.")
                return
            if err.get("code") == 226:
                if attempt < 3:
                    wait = attempt * 45  # 45s, 90s
                    print(f"    ↳ 226 bot-flag (attempt {attempt}/3) — retry dalam {wait}s...")
                    await asyncio.sleep(wait)
                    continue
                raise Exception("RT 226 — terus bot-flagged. Coba: ganti IP/proxy, tambah delay antar akun, atau tunggu ~1 jam.")
            raise Exception(f"RT error [{err.get('code')}]: {err.get('message')}")

        if data.get("data", {}).get("create_retweet", {}).get("retweet_results", {}).get("result"):
            return
...
async def x_like(auth_token: str, ct0: str, my_id: str):
    ...
    if r.status_code == 404:
        raise Exception("Like 404 — queryId FavoriteTweet expired, perlu diupdate.")
    if r.status_code == 401:
        raise Exception("Like 401 — token mati, ganti cookie.")
    data = safe_json(r)
    for err in data.get("errors", []):
        if err.get("code") == 139:
...
async def process_account(acc: dict, idx: int, total: int, state: dict):
    ...
    try:
        ...
        if is_done(state, key, "rt"):
            ...
            secs = random.randint(15, 30)
            print(f"{tag} ⏳ Jeda {secs}s sebelum verify RT...")
...
async def main():
    ...
    print("\033[s", end="", flush=True)  # simpan posisi cursor
    accounts = load_accounts()
    total    = len(accounts)
    state    = load_state()
    print("\033[u\033[J", end="", flush=True)  # balik ke posisi awal, hapus ke bawah
