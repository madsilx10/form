...
URL = "https://docs.google.com/forms/d/e/1FAIpQLSe_fBrSeoiU1ymrg0ktXjK-FLC1i05CZBrFB0V2Sm-AHGsnFQ/formResponse"
...
def submit(handle, eth):
    ...
    try:
        ...
        # Google Forms sukses → halaman HTML mengandung teks konfirmasi
        confirmed = "freebirdFormviewerViewResponseConfirmationMessage" in r.text
        ok = r.status_code in (200, 302) or confirmed
        info = f"HTTP {r.status_code}" + (" ✓confirmed" if confirmed else "")
        return ok, tweet_url, info
...
def main():
    ...
    for i, (handle, eth) in enumerate(pairs, 1):
        ok, tweet_url, info = submit(handle, eth)
        status = "✓" if ok else "✗"
        print(f"[{i}/{total}] {status}  @{handle.lstrip('@')} | {eth[:10]}... | {info} | {tweet_url}")
        if i < total:
            time.sleep(DELAY)
